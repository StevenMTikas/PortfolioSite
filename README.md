...
- **Other disabled project links** — YouTube Product Crew and AI News Aggregator both have `url: null` in `index.html`'s project data, with a comment noting their Render-hosted demos were suspended as of 2026-08-04. Same fate as the old chatbot backend; worth revisiting if either is still worth demoing live. (NFL Picker was removed entirely rather than left disabled, since it's no longer active work.)
