...
More recently, Steve has built a self-directed portfolio of production AI applications: Worxstance (a nine-module AI career workspace SaaS with Stripe billing), SpecAnalyzer (a document-AI pipeline that reads architectural drawing sets for the construction industry), the Tikas Family Digital Home (a self-hosted family platform running local and cloud AI models), and a CrewAI-based YouTube Product Crew multi-agent content pipeline. This work spans OpenAI, Anthropic Claude, Google Gemini, DeepSeek, and local Ollama models, with RAG, structured output, and tool calling.
...
SELECTED PROJECTS
Worxstance — AI-Powered Career Workspace: a nine-module SaaS platform covering job discovery, resume tailoring, skill-gap analysis, interview prep, and offer negotiation. Every AI feature runs through Gemini 2.5 Flash with defined response schemas. Stripe billing, Firestore usage limits, Firebase per-user data isolation. Stack: React, TypeScript, Vite, Firebase, Gemini.
